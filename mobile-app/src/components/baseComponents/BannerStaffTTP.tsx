...
import {useIsStaffMerchant} from '@/hooks/useIsStaffMerchant';

type BannerStaffTTPNavigationProp =
  NativeStackNavigationProp<RootStackParamList>;

const BannerStaffTTP = (): React.JSX.Element | null => {
  const navigation = useNavigation<BannerStaffTTPNavigationProp>();
  const [isVisible, setIsVisible] = useState(true);
  const [isLoading, setIsLoading] = useState(true);
  const {id: userId} = useUserStore();
  const {data: rawDeviceData} = useDeviceStatus();

  const isStaffMerchantSettingsPermission = useIsStaffMerchant();
  ...
  const isShowBanner =
    ...
    isStaffMerchantSettingsPermission &&
    rawDeviceData?.tapToPayStatus ===
