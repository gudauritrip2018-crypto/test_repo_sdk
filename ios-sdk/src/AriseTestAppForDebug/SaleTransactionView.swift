...
struct SaleTransactionView: View {
    ...
    @State private var showAdvancedFields: Bool = true
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 20) {
                // Header
                VStack(spacing: 10) {
                    Text("Submit SALE Transaction")
                        .font(.title)
                        .fontWeight(.bold)

                    Text("Authorize and capture payment in a single step")
                        .font(.subheadline)
                ...
                if showAdvancedFields {
                    amountModifiersSection
                    billingAddressSection
                    contactInfoSection
                }

                calculatedAmountSection
        ...
        .navigationTitle("SALE Transaction")
    ...
    private var amountModifiersSection: some View {
        VStack(alignment: .leading, spacing: 15) {
    ...
    private var calculatedAmountSection: some View {
    ...
    private var submitButton: some View {
        Button(action: submitSaleTransaction) {
            HStack {
                if isLoading {
                    ProgressView()
                        .scaleEffect(0.8)
    ...
    private func resultSection(_ result: CardTransactionResponse) -> some View {
        VStack(alignment: .leading, spacing: 15) {
            HStack {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                if let receipt = result.transactionReceipt {
                    Divider()
                    Text("Receipt")
                        .fontWeight(.semibold)
                        .padding(.top, 8)
                    ...
                    DetailRow(label: "Response", value: receipt.responseDescription ?? "N/A")
                }
    ...
    private func submitSaleTransaction() {
        guard let ariseSdk = ariseSdk else {
        ...
        Task {
            do {
                ...
                let saleResult = try await ariseSdk.submitSaleTransaction(input: input)

                await MainActor.run {
                    self.result = saleResult
                    self.errorMessage = ""
                    self.isLoading = false
                    self.selectedTransactionId = saleResult.transactionId
                }
