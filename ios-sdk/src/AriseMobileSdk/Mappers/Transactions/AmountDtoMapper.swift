...
struct AmountDtoMapper {
    ...
    public static func toModel(_ amount: Components.Schemas.AmountDto?) -> AmountDto {
        return AmountDto(
    ...
    /// Map ISV amount DTO from OpenAPI format to SDK format
    public static func toModel(_ amount: Components.Schemas.IsvAmountDto?) -> AmountDto {
        return AmountDto(
            baseAmount: amount?.baseAmount ?? 0.0,
            percentageOffAmount: amount?.percentageOffAmount ?? 0.0,
            percentageOffRate: amount?.percentageOffRate ?? 0.0,
